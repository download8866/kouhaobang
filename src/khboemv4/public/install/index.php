...
switch ($action)
{
	...
	case 'db_setup':
	{
		if($setup==1){
			...
			if(mysqli_get_server_info($db)>5.7){
			...
			if(mysqli_get_server_info($db)>='5.7'){
				...
				echo "<SCRIPT language=JavaScript>alert('你的mysql版本过低，请确保你的数据库编码为utf-8,官方建议你升级到mysql5.7.0以上');</SCRIPT>";
				die();
				$content=readover("sql.sql");
...
<div class=\"headLogo\">
			<h2 class=\"title\">
				<a href=\"hello\" title=\"hello\">
					<img src=\"{$met_weburl}upload/201801/1515549638.png\" alt=\"hello\" title=\"hello\" />
				</a>
			</h2>
</div>
